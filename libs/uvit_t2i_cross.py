...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., l = 257):
        super().__init__()
    ...
    def forward(self, x):
        B, L, C = x.shape
        query = self.q(x[:,:self.l,:])
        
        kv = self.kv(x)
        if ATTENTION_MODE == 'flash':
            query = einops.rearrange(query, 'B L (H D) -> B H L D',  H=self.num_heads).float()
            kv = einops.rearrange(kv, 'B L (K H D) -> K B H L D', K=2, H=self.num_heads).float()
            q, k, v = query, kv[0], kv[1]  # B H L D
            x = torch.nn.functional.scaled_dot_product_attention(q, k, v)
            x = einops.rearrange(x, 'B H L D -> B L (H D)')
        elif ATTENTION_MODE == 'xformers':
            query = einops.rearrange(query, 'B L (H D) -> B L H D',  H=self.num_heads)
            kv = einops.rearrange(kv, 'B L (K H D) -> K B L H D', K=2, H=self.num_heads)
            q, k, v = query, kv[0], kv[1]  # B L H D
            x = xformers.ops.memory_efficient_attention(q, k, v)
            x = einops.rearrange(x, 'B L H D -> B L (H D)', H=self.num_heads)
        elif ATTENTION_MODE == 'math':
            query = einops.rearrange(query, 'B L (H D) -> B H L D',  H=self.num_heads)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, skip=False, use_checkpoint=False, l = 257):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, l=l)
    ...
    def _forward(self, x, skip=None):
        if self.skip_linear is not None:
            image = self.skip_linear(torch.cat([x[:,:self.l,:], skip], dim=-1))
            x = torch.cat((image, x[:,self.l:,:]), dim =1)
        x = x[:,:self.l,:] + self.attn(self.norm1(x))
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, c=0, v=0, num_heads=12, mlp_ratio=4.,
        ...
        self.image_blocks = nn.ModuleList([
            Block(
                ...
                norm_layer=norm_layer, use_checkpoint=use_checkpoint, l =self.num_patches+1)
            for _ in range(v)])
        
        self.caption_blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                norm_layer=norm_layer, use_checkpoint=use_checkpoint, l=num_clip_token)
            for _ in range(c)])
        ...
        self.joint_skip_blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                norm_layer=norm_layer, skip = skip, use_checkpoint=use_checkpoint, l=self.num_patches+1)
            for _ in range(depth//2)])
